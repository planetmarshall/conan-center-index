...
Prefix = ..""")
    ...
    def _package_rdk_kirkstone_extras(self):
        ...
        # --- libglesbasevertex_shim.so -----------------------------------------
        # LD_PRELOAD trampoline supplying the bare ES 3.2 *BaseVertex entrypoints
        # some SoC GL ES drivers (Broadcom V3D) omit; GLAD's GLES2 loader calls the
        # bare names and the renderer SEGVs when eglGetProcAddress returns NULL.
        # Built once here so consumers bundle it instead of cross-compiling it per
        # widget build. A no-op where the driver exports the bare names (the
        # trampolines forward to them); the launcher gates the LD_PRELOAD on
        # /usr/lib/libv3ddriver.so. -D_GNU_SOURCE for RTLD_NEXT, -ldl for dlsym,
        # --no-as-needed keeps libdl linked.
        gles_shim = os.path.join(lib_dir, "libglesbasevertex_shim.so")
        self.run(f'"{cross_cc}" -shared -fPIC {arm_flags} -D_GNU_SOURCE -O2 '
                 f'-Wl,--no-as-needed -o "{gles_shim}" '
                 f'"{os.path.join(stubs_dir, "glesbasevertex_shim.c")}" -ldl '
                 f'--sysroot="{sysroot}"')
