...
class QtConan(ConanFile):
    ...
    def config_options(self):
        ...
        if Version(self.version) <= "5.6.3":
            ...
            self.options.with_pcre2 = False
            self.options.with_md4c = False
            self.options.with_zstd = False
            ...
            self.options.with_doubleconversion = False
    ...
    def requirements(self):
        ...
        if self.options.with_pq and Version(self.version) > "5.6.3":
...
Prefix = ..""",
        ...
        if self.options.gui:
            ...
            _create_private_module(
                "EventDispatcherSupport", self._event_dispatcher_reqs
            )
    ...
    def package_info(self):
        ...
        if self.options.gui:
            ...
            _add_build_module(
                ...
                self._cmake_qt5_private_file("EventDispatcherSupport"),
            )
            _create_module("FontDatabaseSupport", ["Core", "Gui"])
            _add_build_module(
                "qtFontDatabaseSupport",
                self._cmake_qt5_private_file("FontDatabaseSupport"),
            )
            if self.settings.os == "Windows":
        ...
        if self.options.gui and self.options.get_safe("with_libjpeg"):
            ...
            _create_plugin(
                "QJpegPlugin", "qjpeg", "imageformats", [f"{jpeg_lib}::{jpeg_lib}"]
            )
        ...
        if self.options.gui and self.options.get_safe("qtwayland"):
            ...
            _create_plugin(
                "QWaylandIntegrationPlugin", "qwayland-generic", "platforms", ["Gui"]
            )
            _create_plugin(
    ...
    def _gather_libs(self, p):
        ...
        if (
            ...
            and Version(p.ref.version) >= "17"
        ):
            # libpq/17.x uses Meson filename conventions, pass full filenames instead of "-l" flags
            ext = "lib" if p.options.shared else "a"
            libs = [
                f"lib{lib}.{ext}" for lib in p.cpp_info.aggregated_components().libs
            ]
            libs.extend(
                [f"{lib}.lib" for lib in p.cpp_info.aggregated_components().system_libs]
            )
        if is_apple_os(self):
            libs += [
